...
def test_removal_sets_never_touch_protein_or_nucleic_acids():
...
def test_every_ion_code_maps_to_a_real_element():
...
def test_positions_stay_aligned_with_the_topology():
...
def test_removes_water_and_keeps_the_ligand():
...
def test_removes_bulk_ion_but_keeps_structural_metal(caplog):
    top, positions = _build(
        add_methanol_residue,
...
def test_strips_structural_metal_when_asked():
    top, positions = _build(
        add_methanol_residue,
        lambda t: add_ion_residue(t, "CA", element.calcium, chain_id="C", origin=(-8.0, 0.0, 0.0)),
    )
...
def test_metals_are_silent_when_ion_handling_is_off():
    top, positions = _build(
        add_methanol_residue,
        lambda t: add_ion_residue(t, "CA", element.calcium, chain_id="C", origin=(-8.0, 0.0, 0.0)),
    )
...
def test_removes_crystallization_additives():
...
def test_additives_survive_when_disabled():
    top, positions = _build(add_methanol_residue, lambda t: add_glycerol_residue(t, origin=(0.0, -8.0, 0.0)))
...
def test_never_deletes_a_covalently_bonded_residue(caplog):
...
def test_bonded_copy_is_kept_while_free_copies_go():
    top, positions = _build(
        add_methanol_residue,
...
def test_ion_name_needs_a_single_atom():
...
def test_ion_name_needs_a_matching_element():
    top, positions = _build(
        add_methanol_residue,
...
def test_ion_with_unknown_element_is_kept():
...
def test_four_site_water_is_still_removed():
...
def test_oversized_residue_named_like_water_is_kept():
    def add_fat_hoh(top):
...
def test_residue_names_are_normalized():
...
def test_keep_overrides_the_category():
    top, positions = _build(
        add_methanol_residue,
        lambda t: add_water_residue(t, origin=(5.0, 0.0, 0.0)),
        lambda t: add_glycerol_residue(t, origin=(0.0, -8.0, 0.0)),
...
def test_extra_remove_deletes_a_name_the_tables_do_not_know():
...
def test_extra_remove_refuses_standard_residues():
    top, positions = _build(add_methanol_residue)
    with pytest.raises(ValueError, match="standard residue"):
        clean_topology(top, positions, extra_remove=["ALA"])


def test_warns_about_names_that_match_nothing(caplog):
    top, positions = _build(add_methanol_residue)
    with caplog.at_level(logging.WARNING):
...
def test_all_flags_off_returns_the_inputs_unchanged():
    top, positions = _build(add_methanol_residue, lambda t: add_water_residue(t, origin=(5.0, 0.0, 0.0)))
...
def test_clean_structure_is_a_no_op_on_an_already_clean_topology():
    top, positions = _build(add_methanol_residue)
...
def test_emptied_chains_are_dropped():
    top, positions = _build(add_methanol_residue, lambda t: add_water_residue(t, origin=(5.0, 0.0, 0.0)))
...
def test_ids_bonds_and_box_survive():
    top, positions = _build(add_methanol_residue, lambda t: add_water_residue(t, origin=(5.0, 0.0, 0.0)))
...
def test_removing_everything_warns_and_still_reports(caplog):
...
def test_mismatched_positions_are_rejected():
    top, positions = _build(add_methanol_residue)
...
def test_clean_pdb_roundtrip(tmp_path):
...
def test_clean_pdb_creates_the_parent_directory(tmp_path):
    dirty = write_dirty_pdb(tmp_path / "dirty.pdb")
...
def test_clean_pdb_refuses_to_overwrite_its_input(tmp_path):
    dirty = write_dirty_pdb(tmp_path / "dirty.pdb")
...
def test_clean_pdb_keeps_a_requested_additive(tmp_path):
    dirty = write_dirty_pdb(tmp_path / "dirty.pdb")
    out = tmp_path / "clean.pdb"
...
def test_module_exports_the_name_tables():
