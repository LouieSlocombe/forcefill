...
def test_single_file_derives_its_residue_name(fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path) -> None:
...
def test_mapping_names_the_residue_explicitly(fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path) -> None:
...
def test_sequence_of_files(fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path) -> None:
...
def test_colliding_derived_names_are_refused(tmp_path: Path) -> None:
...
def test_per_ligand_settings_reach_antechamber(
...
def test_net_charge_is_read_from_the_ligand_file(
...
def test_explicit_net_charge_contradicting_the_file_is_refused(
    fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path
) -> None:
    ben = Path(__file__).parent.parent / "examples" / "data" / "benzamidinium.sdf"
...
def test_a_ligand_with_no_source_is_refused(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="no source"):
        build_ligand_xml({"LIG": LigandSpec()}, tmp_path / "out.xml", workdir=tmp_path / "wd")


def test_a_bare_smiles_spec_needs_a_name(tmp_path: Path) -> None:
...
def test_no_ligands_is_refused(tmp_path: Path) -> None:
...
def test_bad_options_rejected_early(tmp_path: Path, kwargs: Mapping[str, str], match: str) -> None:
    with pytest.raises(ValueError, match=match):
        build_ligand_xml("lig.sdf", tmp_path / "out.xml", **kwargs)


def test_validation_uses_the_molecule_as_its_own_topology(
    fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path
) -> None:
...
def test_cleanup_removes_the_workdir(fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path) -> None:
    sdf = write_methanol_sdf(tmp_path / "lig.sdf")
    result = build_ligand_xml(
...
def test_cleanup_refuses_output_inside_the_workdir(
    fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path
) -> None:
    sdf = write_methanol_sdf(tmp_path / "lig.sdf")
...
def test_failure_keeps_the_workdir(
...
def test_a_pdb_ligand_file_still_works(fake_ambertools: dict[str, list[dict[str, Any]]], tmp_path: Path) -> None:
...
def test_public_api_exposes_the_entry_point() -> None:
