...
Requires AmberTools on PATH for the GAFF half, and openff-toolkit +
openmmforcefields for the SMIRNOFF half (conda install -c conda-forge ambertools
openff-toolkit openmmforcefields). Either half is skipped if its stack is absent.
"""
...
def main():
    ...
    if shutil.which("antechamber"):
        ...
        print("skipping the gaff backend: antechamber is not on PATH")

    if _have_openff():
        run("smirnoff", HERE / "ben_standalone_smirnoff.xml", HERE / "wd_standalone_smirnoff")
    else:
        print("skipping the smirnoff backend: openff-toolkit is not installed")
    ...
    if shutil.which("antechamber") and _have_openff():
        print("\n--- mixed, in one XML ---")
        result = build_ligand_xml(
            {
                "BEN": LigandSpec(file=BEN_SDF, backend="smirnoff"),
                "MOL": LigandSpec(smiles="CO", backend="gaff"),  # needs rdkit
            },
